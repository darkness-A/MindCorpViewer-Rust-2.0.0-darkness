...
impl Screenshot {
    ...
    pub fn save(&mut self, window_size: [GLint; 2]) {
        unsafe {
            ...
            let mut full_path: Option<PathBuf> = None; // 定义一个 Option 类型变量
            if let Some(desktop_path) = dirs::desktop_dir() {
                full_path = Some(desktop_path.join(file_name)); // 将路径赋值给 full_path
